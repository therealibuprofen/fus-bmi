function model = train_classifier_cnn_lstm_twohead(trainData, trainLabels, varargin)
% train_classifier_cnn_lstm_twohead  Train a shared-backbone two-head CNN+LSTM decoder.
...
if size(trainLabels, 2) ~= 2
    error('Two-head CNN+LSTM requires trainLabels to be N x 2.');
end
...
warmStartUsed = false;
if has_valid_initial_model(cfg.initial_model, 'CNN_LSTM_TWOHEAD')
...
model = struct();
model.method = 'CNN_LSTM_TWOHEAD';
model.baseMethod = 'CNN+LSTM';
model.dlnet = dlnet;
...
tail = [
    ...
    fullyConnectedLayer(max(32, cfg.lstm_units), 'Name', 'shared_fc')
...
lg = addLayers(lg, fullyConnectedLayer(nHorz, 'Name', 'fc_horz'));
lg = addLayers(lg, fullyConnectedLayer(nVert, 'Name', 'fc_vert'));
...
for epoch = 1:maxEpochs
    ...
    for iter = 1:numIterationsPerEpoch
        ...
        dlX = dlarray(permute(XTrain(:, :, :, :, batchIdx), [1 2 4 3 5]), 'SSCTB');
        if use_gpu(cfg.execution_environment)
            dlX = gpuArray(dlX);
        end

        [gradients, state, loss] = dlfeval(@model_gradients, dlnet, dlX, ...
            yTrainH(batchIdx), yTrainV(batchIdx), weightsH, weightsV);
        dlnet.State = state;
    ...
    if cfg.verbose
        fprintf('[train_classifier_cnn_lstm_twohead] epoch %d/%d, metric=%.4f\n', epoch, maxEpochs, metric);
    end
...
for i = 1:numBatches
    ...
    dlX = dlarray(permute(XVal(:, :, :, :, batchIdx), [1 2 4 3 5]), 'SSCTB');
    if use_gpu(cfg.execution_environment)
        dlX = gpuArray(dlX);
    end
...
countSafe = max(count, 1);
mu = sumX ./ countSafe;
varX = max(sumX2 ./ countSafe - mu.^2, 0);
sigma = sqrt(varX);
sigma(sigma == 0) = 1;
mu = single(mu);
sigma = single(sigma);
