...
if isfield(data, 'tensor_prepool') && ~isempty(data.tensor_prepool)
    ...
    % Real-time friendly default for tensor decoders: denoise + smaller input.
    tensor_prepool = is_tensor_decoder;
end
...
% Optional FCNN training hyperparams for real-time
if isfield(data, 'fcnn_params') && ~isempty(data.fcnn_params)
    fcnn_params = data.fcnn_params;
else
    fcnn_params = struct( ...
        'hidden_dims', [128 64], ...
        'dropout', 0.3, ...
...
if isfield(data, 'cnn_params') && ~isempty(data.cnn_params)
    ...
    if nClasses == 2
        cnn_num_filters = [8 16 32];
        cnn_dropout = 0.15;
        cnn_max_epochs = 80;
        cnn_validation_ratio = 0.05;
        cnn_class_weight_mode = 'none';
        cnn_fine_tune_epochs = 8;
    else
        cnn_num_filters = [8 16 32];
        cnn_dropout = 0.2;
        cnn_max_epochs = 70;
        cnn_validation_ratio = 0.1;
        cnn_class_weight_mode = 'balanced';
        cnn_fine_tune_epochs = 6;
    end
    cnn_params = struct( ...
        ...
        'gradient_threshold_method', 'l2norm', ...
        'learn_rate_schedule', 'piecewise', ...
        'learn_rate_drop_factor', 0.5, ...
        'learn_rate_drop_period', 15, ...
        'validation_patience', 8, ...
        'output_network', 'best-validation-loss', ...
        'validation_ratio', cnn_validation_ratio, ...
        'class_weight_mode', cnn_class_weight_mode, ...
        'normalize_in_network', true, ...
        'execution_environment', 'gpu', ...
        'fine_tune_epochs', cnn_fine_tune_epochs ...
    );
end
...
if isfield(data, 'cnn_lstm_params') && ~isempty(data.cnn_lstm_params)
    ...
    if nClasses == 2
        cnn_lstm_dropout = 0.15;
        cnn_lstm_units = 48;
        cnn_lstm_max_epochs = 70;
        cnn_lstm_validation_ratio = 0.05;
        cnn_lstm_class_weight_mode = 'none';
        cnn_lstm_fine_tune_epochs = 8;
    else
        cnn_lstm_dropout = 0.2;
        cnn_lstm_units = 64;
        cnn_lstm_max_epochs = 60;
        cnn_lstm_validation_ratio = 0.1;
        cnn_lstm_class_weight_mode = 'balanced';
        cnn_lstm_fine_tune_epochs = 6;
    end
    cnn_lstm_params = struct( ...
        ...
        'max_epochs', cnn_lstm_max_epochs, ...
        'mini_batch_size', 8, ...
        'initial_learn_rate', 1e-3, ...
        'l2_regularization', 1e-4, ...
        'gradient_threshold', 1.0, ...
        ...
        'validation_patience', 8, ...
        'output_network', 'best-validation-loss', ...
        'validation_ratio', cnn_lstm_validation_ratio, ...
        'class_weight_mode', cnn_lstm_class_weight_mode, ...
        'normalize_in_network', true, ...
        'execution_environment', 'gpu', ...
        'fine_tune_epochs', cnn_lstm_fine_tune_epochs ...
    );
end
...
if phase(k) == 4 && ...
    ...
    if is_fcnn_decoder || (is_tensor_decoder && tensor_prepool)
...
function args = fcnn_train_args(p)
    % Convert struct to name-value pairs for train_decoder
    args = { ...
...
function args = cnn_train_args(p)
    ...
    if ~isfield(p, 'gradient_threshold') || isempty(p.gradient_threshold)
        p.gradient_threshold = 1.0;
    end
    if ~isfield(p, 'gradient_threshold_method') || isempty(p.gradient_threshold_method)
        p.gradient_threshold_method = 'l2norm';
    end
    if ~isfield(p, 'validation_patience') || isempty(p.validation_patience)
        p.validation_patience = 8;
    end
    if ~isfield(p, 'output_network') || isempty(p.output_network)
        p.output_network = 'best-validation-loss';
    end
    if ~isfield(p, 'validation_ratio') || isempty(p.validation_ratio)
        p.validation_ratio = 0.1;
    end
    args = { ...
        ...
        'gradient_threshold_method', p.gradient_threshold_method, ...
        'learn_rate_schedule', p.learn_rate_schedule, ...
        'learn_rate_drop_factor', p.learn_rate_drop_factor, ...
        'learn_rate_drop_period', p.learn_rate_drop_period, ...
        'validation_patience', p.validation_patience, ...
        'output_network', p.output_network, ...
...
if k==nb_block
    ...
    if save_data && ~isempty(decode_latency_ms)
        ...
        latencySummaryPath = fullfile(data_savepath, 'decode_latency_summary.mat');
        save(latencySummaryPath, 'decode_latency');
        fprintf('[predict_movement_direction] Saved decode latency summary to %s: mean=%.2f ms, median=%.2f ms, p95=%.2f ms, n=%d\n', ...
            latencySummaryPath, latMean, latMedian, latP95, numel(decode_latency_ms));
    end
end
...
%% Initialize workspace
    function initialize_workspace()
        ...
        if save_data
            ...
            fprintf('[predict_movement_direction] Logging summary to: %s\n', ...
                fullfile(data_savepath, 'summaryResults.txt'));
            ...
            save(fullfile(data_savepath, 'experiment_parameters.mat'), 'task_type', ...
                ...
                'hrf_params', 'prediction_smoothing_window', 'tensor_prepool');
        end
        ...
        if use_previous_data_session
            ...
            elseif is_cnn_decoder || is_cnn_lstm_decoder
                ...
                if tensor_prepool
                    train = max_pool_flattened_trials(train, m_prev, n_prev, training_set_size);
                    train_tensor = max_pool_tensor_trials(train_tensor);
                    fprintf('[predict_movement_direction] Applied 2x2 max-pool to pretrain tensor for %s\n', decoder_method);
                end
...
function tensor_out = max_pool_tensor_trials(tensor_in)
    [~, ~, nFrames, nTrials] = size(tensor_in);
    pooled0 = max_pool_2x2_stride2_3d(tensor_in(:, :, :, 1));
    [m2, n2, ~] = size(pooled0);
    tensor_out = zeros(m2, n2, nFrames, nTrials, 'like', tensor_in);
    tensor_out(:, :, :, 1) = pooled0;
    for t = 2:nTrials
        tensor_out(:, :, :, t) = max_pool_2x2_stride2_3d(tensor_in(:, :, :, t));
    end
end
...
% files...
    ...
    function plot_confusion_matrix(subplot_start)
        ...
        confusion_chart = create_confusion_matrix(reordered_counting_matrix(1:8, :), ...
            ...
            'title', sprintf('Combined prediction (Decoder: %s)', decoder_method), ...
            'FontColor', 'w');
    ...
    function plot_split_performance(subplot_start)
        ...
        title(sprintf('Horizontal accuracy (Decoder: %s)', decoder_method), ...
            'Color', 'w');
        ...
        text(1, correct_count_vert*1.01, sprintf('%2.1f%%',correct_percent_vert),...
            'VerticalAlignment','bottom','HorizontalAlignment','center')
        title(sprintf('Vertical accuracy (Decoder: %s)', decoder_method), ...
            'Color', 'w');
    ...
    function plot_counter(subplot_start)
        ...
        text(1, correct_count*1.01, sprintf('%2.1f%%',correct_percent),...
            'VerticalAlignment','bottom','HorizontalAlignment','center')
        title(sprintf('Overall accuracy (Decoder: %s)', decoder_method), ...
            'Color', 'w');
