...
is_shared_twohead_method = strcmpi(sets.classificationMethod, 'CNN') || ...
    strcmpi(sets.classificationMethod, 'CNN+LSTM') || ...
    strcmpi(sets.classificationMethod, 'CNN_LSTM') || ...
    strcmpi(sets.classificationMethod, 'CNNLSTM');
...
for i = 1:sets.K
    ...
    % Train models
    use_shared_twohead = is_shared_twohead_method && ...
        isequal(horz_train, vert_train) && isequal(horz_test, vert_test) && ...
        any(horz_train) && any(horz_test);

    if use_shared_twohead
        model_shared = train_decoder(data(horz_train, :), ...
            labels(horz_train, :), ...
            'method', sets.classificationMethod, ...
            'variance_to_keep', sets.variance_to_keep);
        class_pair = predict_decoder(data(horz_test, :), model_shared);
        class_horz = class_pair(:, 1);
        class_vert = class_pair(:, 2);
    else
